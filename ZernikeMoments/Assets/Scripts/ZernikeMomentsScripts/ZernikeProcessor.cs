...
public class ZernikeProcessor
{
    ...
    // Inicializa el procesador con el tamano de la imagen.
    ...
    {
        // Asegurar que los puntos estan dentro de los limites de la imagen
        ...
        if (steep)
        {
            // Intercambiar x e y para manejar lineas con pendiente > 1
    ...
    // Funcion auxiliar para asignar intensidad a un pixel con seguridad
    private void SetPixelSafe(int x, int y, float intensity)
    {
        if (x >= 0 && x < _imageSize && y >= 0 && y < _imageSize)
        {
            // Acumular intensidad (suma en lugar de sobrescribir, util para lineas que se cruzan)
    ...
    public void DrawStrokes(List<List<Vector2>> allStrokes)
    {
        ...
        // 2. Encuentra los limites de TODOS los puntos para una normalizacion correcta
    ...
    public void DrawTexture(Texture2D texture)
    {
        ...
        {
            Debug.LogError("La textura no es legible. Activa Read/Write Enabled en la importacion.");
            return;
        }
        ...
        // 1. Encontrar bounding box del simbolo en la textura
        ...
        for (int y = 0; y < _imageSize; y++)
        {
            for (int x = 0; x < _imageSize; x++)
            {
                ...
                // Si esta fuera del bounding box, pixel negro (0)
                ...
                {
                    // Tomamos el pixel interpolado (bilineal) para suavizar, o nearest neighbor si queres linea de 1px exacta
    ...
    // Calcula los momentos de Zernike hasta un orden maximo maxN.
    public ZernikeMoment[] ComputeZernikeMoments(int maxN)
    {
        ...
        // 3) calculo de momentos (usando double para mas precision)
    ...
    // Funcion principal: devuelve una matriz con grosor uniforme (targetThickness en pixeles)
